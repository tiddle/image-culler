...
# Exposure/blur tests isolate pixel math from the MediaPipe blink stage.
...
def test_well_exposed_sharp_image_is_kept(tmp_path: Path) -> None:
    ...
    verdict = analyze_image(p, NO_BLINK)
    assert verdict.keep
...
def test_thresholds_are_tunable(tmp_path: Path) -> None:
    ...
    strict = analyze_image(p, Thresholds(min_blur_var=100.0, check_blink=False))
    lenient = analyze_image(p, Thresholds(min_blur_var=0.0, check_blink=False))
